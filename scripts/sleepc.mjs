...
(async () => {
    ...
    const negativeRealTimeQuestion =
    ...
    const timeInBed = await getTimeInBed();
    const timeBeforeAsleep = await getTimeBeforeAsleep();
    const realTime = timeInBed - timeBeforeAsleep;

    if (Math.sign(realTime) === -1) {
        console.log(negativeRealTimeWarn);
        process.exit(1);
    }

    sleepPoints = 
	calculateAcceptableTimeBeforeAsleepPoints(sleepPoints, timeBeforeAsleep);

    sleepPoints = calculateTimeInBedPoints(sleepPoints, realTime);

    console.log(`Você teve uma qualidade de sono de: ${sleepPoints}!`);
